    ...
    private void buildK2Paths() throws IOException {
        ...
        for(long pathId : pathMap.values()){
            ...
            while(result.hasNext(cursor)){
                ...
                cursor.close();
                enumerateAllPathsFrom(entry);
                cursor = indexes.get(1).disk.getCursor(result.pageID, PagedFile.PF_SHARED_LOCK);
            }
        }
    }

    private void enumerateAllPathsFrom(long[] key) throws IOException {
        for(long pathId : pathMap.values()) {
            ...
            try (PageProxyCursor cursor = indexes.get(1).disk.getCursor(0, PagedFile.PF_SHARED_LOCK)) {
                while (result.hasNext(cursor)) {
                    long[] secondPath = result.next(cursor);
                    long[] combinedPath = new long[2 + (key.length - 2) + (secondPath.length - 2)];
