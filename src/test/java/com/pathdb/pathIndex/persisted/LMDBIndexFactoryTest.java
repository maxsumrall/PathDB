...
public class LMDBIndexFactoryTest {
    ...
    public void testInsertionOrderAndRetrievalOrder() throws IOException {
        ...
        for( int i = 0; i < 10; i++){
            int count = 1;
            Iterable<Path> paths = db.getPaths(ImmutablePathPrefix.builder().length(3).pathId(i).build());
    ...
    public void openInsertCloseOpenRead() throws Exception {
        ...
        PathPrefix pathPrefix =
                ImmutablePathPrefix.builder().pathId(42L).length(4).addNodes(new Node(42)).build();
